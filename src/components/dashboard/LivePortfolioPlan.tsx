...
export function LivePortfolioPlan() {
    ...
    series.forEach(p => {
        ...
        const m = months.get(key) || { value: 0, invested: 0, factor: 1, observations: 0, drawdown: 0 };
        m.value = p.value;
        m.invested = p.invested;
        m.drawdown = Math.min(m.drawdown, (p.index / peak - 1) * 100);
        ...
        <Card className="live-plan">
            ...
            <CardContent>
                <div className="live-plan__scroll"><table>
                    ...
                    <tbody>{[...months].reverse().map(([month, m]) => <tr key={month}><th scope="row">{month}</th><td>{money(m.value)}</td><td>{money(m.invested)}</td><td>{m.observations ? pct((m.factor - 1) * 100) : 'N/D'}</td><td>{m.observations ? pct(m.drawdown) : 'N/D'}</td><td>{m.observations}</td></tr>)}</tbody>
                </table></div>
                <p>*Desde la primera valoración disponible. Los periodos sin datos o con correcciones de posición se excluyen.</p>
            </CardContent>
        </Card>
